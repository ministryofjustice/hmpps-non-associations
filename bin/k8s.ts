...
export const namespaces = Object.fromEntries(
  environments.map(environment => [environment, `hmpps-non-associations-${environment}`]),
)

function readJsonFromSubprocess<T>(subprocess: ChildProcessByStdio<null, Readable, Readable>): Promise<T> {
...
function kubectl(namespace: string, args: string[], captureJson?: true): ChildProcessByStdio<null, Readable, Readable>
function kubectl(namespace: string, args: string[], captureJson: false): ChildProcessByStdio<null, null, Readable>
function kubectl(namespace: string, args: string[], captureJson = true) {
